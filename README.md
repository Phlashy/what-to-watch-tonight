...
- **Tonight** — The main decision screen. Shows what's on the watchlist for tonight's context (family night, solo, etc.) with drag-to-reorder, a Hide button, shortlist stars, and a "Pick for me" random selector
...
- **Optional password protection** — off by default; set one env var to require a shared password if you expose the app beyond your LAN
...
| `npm run db:pull` / `db:check` / `db:push` | Sync the local DB with production (see [docs/DATA-AND-BACKUPS.md](docs/DATA-AND-BACKUPS.md)) |
...
- `ANTHROPIC_API_KEY` — From [Anthropic Console](https://console.anthropic.com/) (required for the chat feature)
- `APP_PASSWORD` _(optional)_ — set a shared password to require sign-in; unset by default for zero-friction LAN use
- `CORS_ORIGIN` _(optional)_ — comma-separated CORS allowlist; permissive by default
- `PORT`, `DB_PATH` _(optional)_ — overrides

## Documentation

- [docs/WORKFLOW.md](docs/WORKFLOW.md) — the development loop (read this first)
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — system design, schema (ER diagram), API surface
- [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md) — dev setup, code style, testing, PRs
- [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) — local, Raspberry Pi, and cloud
- [docs/DATA-AND-BACKUPS.md](docs/DATA-AND-BACKUPS.md) — database sync & backups
- [docs/AUDIT-OUTCOME.md](docs/AUDIT-OUTCOME.md) — the 2026 audit: why, what, lessons (complete)
- [CHANGELOG.md](CHANGELOG.md) · [KNOWN-ISSUES.md](KNOWN-ISSUES.md)

CI (GitHub Actions) runs on every push: install · lint · format · test · build · audit.
...
> **A note on history:** the git history was reset to a single commit in June 2026. Earlier development commits contained local paths and personal details that didn't belong in a public repo, so the repo starts clean from its post-audit state. The story of how it got here lives in [docs/AUDIT-OUTCOME.md](docs/AUDIT-OUTCOME.md) and [CHANGELOG.md](CHANGELOG.md).
