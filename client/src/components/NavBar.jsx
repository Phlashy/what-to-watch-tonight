...
const tabs = [
  {
    ...
    icon: (active) => (
      <svg
        className={`w-6 h-6 ${active ? 'text-amber-400' : 'text-slate-400'}`}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={1.8}
      ...
      <div className="flex items-stretch">
        {visibleTabs.map((tab) => (
          <NavLink key={tab.to} to={tab.to} end={tab.to === '/'} className="flex-1">
            {({ isActive }) => (
              <div
                className={`flex flex-col items-center justify-center py-2 transition-colors ${isActive ? 'text-amber-400' : 'text-slate-400'}`}
              >
                {tab.icon(isActive)}
                <span
                  className={`text-xs mt-1 font-medium ${isActive ? 'text-amber-400' : 'text-slate-500'}`}
                >
                  {tab.label}
                </span>
              </div>
        ...
        <button
          onClick={openPicker}
          aria-label={
            currentPerson ? `Current person: ${currentPerson}. Switch person` : 'Choose who you are'
          }
