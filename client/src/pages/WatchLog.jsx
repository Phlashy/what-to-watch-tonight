...
const STATUS_COLORS = {
  wishlist: 'bg-slate-700 text-slate-300',
  ...
  dropped: 'bg-slate-700 text-slate-500',
};
    ...
    <Link to={`/title/${v.title_id}`} state={fromState} className="block">
      <div className="flex gap-3 p-3 bg-slate-800 rounded-xl hover:bg-slate-750 active:bg-slate-700 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          {watchers.length > 0 && (
            <div className="text-xs text-slate-400 mt-0.5">Watched by {watchers.join(', ')}</div>
          )}
          {v.picked_by && (
            <div className="text-xs text-slate-400 mt-0.5">Chosen by {v.picked_by}</div>
          )}
          {tags.length > 0 && (
          ...
          {v.notes && (
            <p className="text-xs text-slate-400 mt-1 italic line-clamp-2">"{v.notes}"</p>
          )}
        </div>
      </div>
    </Link>
...
function ShowRow({ show }) {
  ...
  const lastWatchedLabel = show.last_watched
    ? new Date(show.last_watched + 'T00:00:00').toLocaleDateString('en-CA', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      })
  ...
  return (
    <Link to={`/title/${show.title_id}`} state={fromState} className="block">
      <div className="flex gap-3 p-3 bg-slate-800 rounded-xl hover:bg-slate-750 active:bg-slate-700 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-semibold text-slate-100 text-sm leading-tight">{show.title}</h3>
            <span className="flex-shrink-0 text-xs text-slate-500">
              {show.viewing_count} {show.viewing_count === 1 ? 'session' : 'sessions'}
            </span>
          </div>
          {lastWatchedLabel && (
            <div className="text-xs text-slate-500 mt-0.5">Last watched {lastWatchedLabel}</div>
          )}
          {uniqueWatchers.length > 0 && (
            <div className="text-xs text-slate-400 mt-0.5">
              Watched by {uniqueWatchers.join(', ')}
            </div>
          )}
          {statuses.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {statuses.map((s) => (
                <span
                  key={s.person}
                  className={`text-xs rounded-full px-2 py-0.5 font-medium ${STATUS_COLORS[s.status] || 'bg-slate-700 text-slate-400'}`}
                >
                  {s.person} · {STATUS_LABELS[s.status] || s.status}
                </span>
              ))}
            </div>
          )}
...
export default function WatchLog() {
  ...
  useEffect(() => {
    ...
    // loadMovies/loadShows also read the page state, but this effect must fire on
    // filter changes only (it resets to page 1; "Load more" drives later pages) —
    // so the loaders are deliberately not dependencies.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [search, person, minRating, tags, hasNotes, sort, tab]);

  async function loadMovies(p = moviesPage, reset = false) {
    setMoviesLoading(true);
    const params = new URLSearchParams({ page: p, limit: 40, type: 'movie' });
    ...
    try {
      ...
      setViewings((v) => (reset ? items : [...v, ...items]));
      setMoviesHasMore(items.length === 40);
    } finally {
      setMoviesLoading(false);
    }
  }

  async function loadShows(p = showsPage, reset = false) {
    setShowsLoading(true);
    const params = new URLSearchParams({ page: p, limit: 40 });
    if (search) params.set('search', search);
    try {
      const res = await api(`/api/show-status/log?${params}`);
      const data = await res.json();
      const items = data.shows || [];
      setShows((s) => (reset ? items : [...s, ...items]));
      setShowsHasMore(items.length === 40);
    } finally {
      setShowsLoading(false);
    }
  }

  const loading = tab === 'movies' ? moviesLoading : showsLoading;
  const hasMore = tab === 'movies' ? moviesHasMore : showsHasMore;
  ...
  return (
    <div className="pb-safe">
        ...
        <div className="px-4 pt-safe pb-3">
          ...
          <div className="flex rounded-xl bg-slate-800 p-1 mb-3">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${tab === 'movies' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-slate-300'}`}
            >
              🎬 Movies
            </button>
            <button
              onClick={() => setTab('shows')}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${tab === 'shows' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-slate-300'}`}
            >
              📺 Shows
            </button>
          </div>
          ...
          <div className="flex gap-2">
            ...
            {/* Filters button — only shown for movies (shows tab has no person/rating filter for now) */}
            {tab === 'movies' && (
              <button
          ...
          {/* Movies-only: sort + has-notes quick row */}
          {tab === 'movies' && (
          ...
          {/* Movies-only: expanded filters */}
          {tab === 'movies' && showFilters && (
      ...
      <div className="px-4 pt-3 space-y-2">
        {tab === 'movies' && viewings.map((v) => <ViewingRow key={v.id} v={v} />)}
        {tab === 'shows' && shows.map((s) => <ShowRow key={s.title_id} show={s} />)}

        {loading &&
          [1, 2, 3].map((i) => (
            <div key={i} className="bg-slate-800 rounded-xl h-20 animate-pulse" />
          ))}
        {!loading && hasMore && (tab === 'movies' ? viewings : shows).length > 0 && (
          <button
            onClick={loadMore}
            className="w-full text-center text-sm text-slate-400 hover:text-slate-300 py-4"
          >
            Load more
          </button>
        )}
        {!loading && (tab === 'movies' ? viewings : shows).length === 0 && (
          <div className="text-center py-16 text-slate-500">
            <div className="text-4xl mb-3">{tab === 'movies' ? '🎬' : '📺'}</div>
            <p className="font-medium">No {tab === 'movies' ? 'movies' : 'shows'} found</p>
          </div>
        )}
      </div>
