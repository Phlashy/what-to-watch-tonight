...
const merge = db.transaction((group) => {
  ...
  for (const otherId of others) {
    ...
    const movedV = db
      .prepare('UPDATE viewings SET title_id = ? WHERE title_id = ?')
      .run(canonical.id, otherId);
    const movedL = db
      .prepare('UPDATE list_items SET title_id = ? WHERE title_id = ?')
      .run(canonical.id, otherId);
    ...
    console.log(
      `  Merged: "${other.title}" (id ${otherId}) → "${canonical.title}" (id ${canonical.id})`
    );
...
console.log(
  `Done. Removed ${mergedTitles} duplicate title record(s), moved ${mergedViewings} viewing(s) and ${mergedListItems} list item(s).`
);
