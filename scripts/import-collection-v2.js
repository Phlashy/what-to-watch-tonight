...
db.prepare(
  "UPDATE collection SET notes = 'The Ultimate Edition + standalone Goes Forth' WHERE title_id = 560 AND format = 'dvd'"
).run();
db.prepare(
  "UPDATE collection SET notes = 'Special Collector''s Edition + Standard Edition (2 copies)' WHERE title_id = (SELECT id FROM titles WHERE title LIKE '%Trainspotting%' LIMIT 1) AND format = 'dvd'"
).run();
...
for (const [titleId, format, notes] of EXISTING_DB) {
  try {
    db.prepare('INSERT INTO collection (title_id, format, notes) VALUES (?, ?, ?)').run(
      titleId,
      format,
      notes
    );
...
async function enrichTitle(id, title, type, searchOverride) {
  ...
  const detailRes = await fetch(
    `https://api.themoviedb.org/3/${tmdbType}/${match.id}?api_key=${API_KEY}&append_to_response=credits`
  );
...
async function main() {
  ...
  for (const [title, type, format, notes, searchOverride] of NEW_TITLES) {
    ...
    try {
      ...
      if (info) {
        console.log(
          `  ✓ ${title} (${info.year || '?'}) — ${info.director || 'no director'} [${format.toUpperCase()}]`
        );
      } else {
