...
function findTitle(searchTerm) {
  // Exact match first
  let row = db
    .prepare('SELECT id, title FROM titles WHERE title = ? COLLATE NOCASE')
    .get(searchTerm);
  if (row) return row;

  // Contains match
  row = db
    .prepare('SELECT id, title FROM titles WHERE title LIKE ? COLLATE NOCASE')
    .get(`%${searchTerm}%`);
  if (row) return row;
...
for (const [searchTerm, format, notes] of SHELF_TITLES) {
  ...
  if (found) {
    ...
    console.log(
      `  ✓ ${added ? 'Added' : 'Already'}: "${found.title}" (ID:${found.id}) as ${format.toUpperCase()}`
    );
  } else {
    ...
    console.log(
      `  + Created & added: "${newTitle.title}" (ID:${newTitle.id}) as ${format.toUpperCase()}`
    );
  }
}
...
console.log(`\n=== Summary ===`);
console.log(
  `Matched existing: ${matched.length} (${matched.filter((m) => m.added).length} new, ${matched.filter((m) => !m.added).length} already in collection)`
);
