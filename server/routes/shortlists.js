...
router.post('/', (req, res) => {
  ...
  if (!title_id || !person || !context)
    return res.status(400).json({ error: 'title_id, person, context required' });

  const existing = db
    .prepare('SELECT id FROM shortlists WHERE title_id = ? AND person = ? AND context = ?')
    .get(title_id, person, context);
  if (existing) {
    db.prepare('DELETE FROM shortlists WHERE id = ?').run(existing.id);
    return res.json({ action: 'removed', title_id, person, context });
  }

  db.prepare('INSERT INTO shortlists (title_id, person, context) VALUES (?, ?, ?)').run(
    title_id,
    person,
    context
  );
...
router.delete('/:id', (req, res) => {
  const db = req.app.locals.db;
  const { changes } = db.prepare('DELETE FROM shortlists WHERE id = ?').run(req.params.id);
  if (!changes) return res.status(404).json({ error: 'Shortlist entry not found' });
