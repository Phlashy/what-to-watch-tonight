...
router.post('/', (req, res) => {
  ...
  try {
    const result = db
      .prepare('INSERT INTO collection (title_id, format, platform, notes) VALUES (?, ?, ?, ?)')
...
router.delete('/:id', (req, res) => {
  const db = req.app.locals.db;
  const { changes } = db.prepare('DELETE FROM collection WHERE id = ?').run(req.params.id);
  if (!changes) return res.status(404).json({ error: 'Collection entry not found' });
