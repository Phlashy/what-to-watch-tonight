...
// Run migrations (same runner the server uses)
require('../server/lib/migrate').migrate(db);
...
const insertPerson = db.prepare(
  'INSERT OR IGNORE INTO people (name, display_name, aliases) VALUES (?, ?, ?)'
);
...
const insertTitle = db.prepare(
  'INSERT OR IGNORE INTO titles (title, title_raw, type) VALUES (?, ?, ?)'
);
...
const insertViewingsTx = db.transaction(() => {
  ...
  for (const entry of allWatched) {
    ...
    if (!titleId) {
      console.warn(`  ! No title found: "${entry.title}"`);
      continue;
    }

    const notes = seedData.reviews[entry.title] || null;
    const tags = JSON.stringify(entry.tags || []);
    const res = insertViewing.run(
      titleId,
      entry.date || null,
...
const insertListsTx = db.transaction(() => {
  ...
  for (const entry of allToWatch) {
    ...
    if (!titleId) {
      console.warn(`  ! No title for list item: "${entry.title}"`);
      continue;
    }
    if (!listId) {
      console.warn(`  ! No list: "${entry.list}"`);
      continue;
    }
